...
class selectPersonController: UIViewController,UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate {
    ...
    func midviewSetting(){
        ...
        //right button
        var buttonright = UIButton(frame: CGRectMake(width - 40, 15, 25, 25))
        buttonright.setImage(UIImage(named: "im_select.png"), forState: UIControlState.Normal)
        buttonright.addTarget(self, action: "ojpress:", forControlEvents: UIControlEvents.TouchUpInside)
    ...
    func pressshowall(sender:UIButton){
        ...
        con.rowFlag = self.rowflag
        con.flag = 0
        con.allID = self.allID
    ...
    func press443(sender:UIButton){
        var tm = sender.titleLabel?.text
        if(tm != nil){
            var tm1 : NSString = tm! as NSString
            var cm = tm1.substringToIndex(2)
            if(cm == "1~"){
                sender.titleLabel?.text = tm1.substringFromIndex(2)
                var leftinfo = sender.titleLabel?.text!
                self.allID.removeObject(leftinfo!)
                sender.backgroundColor = UIColor(patternImage: UIImage(named: "im_unselect.png")!)
            }else{
                if(self.allID.containsObject(tm!)){
                    
                }else{
                    self.allID.addObject(tm!)
                }
                sender.backgroundColor = UIColor(patternImage: UIImage(named: "im_select.png")!)
                sender.titleLabel?.text = "1~\(tm1)"
            }
        }
    }
