...
//  itjh
//
//  Created by huasu on 15/3/27.
//  Copyright (c) 2015 zwb. All rights reserved.
//

import UIKit
import Alamofire
class selectCustomerViewController: UIViewController,UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate {
    ...
    //
    var nowSelectedName = String()
    var nowSelectedID = String()
    var nowContent = String()
    var nowSegementItem = 0
    var nowResult = String()
    var nowPromise = String()
    var nowUserID = String()
    var selectedDay = String()
    var plan_resultItem = 0
    ...
    func midviewSetting(){
        ...
        label.text = "可选人员"
        label.textColor = _globalTitleColor
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        if(tableView.tag == 1){
            ...
            var iddata = self.arraylist[indexPath.row] as NSDictionary
            var con = AppendPlanViewController()
            ...
            con.workContent = nowContent
            con.resultDefine = nowResult
            con.promisePunish = nowPromise
            con.selfItem = nowSegementItem
            
            con.nowDay = self.selectedDay
            con.nowID = self.nowUserID
            con.segementItem = self.plan_resultItem
    ...
    func pressshowall(sender:UIButton){
        var con = AppendPlanViewController()
        con.modalTransitionStyle = _globalCustomviewchange
        con.receiverName = nowSelectedName
        con.receiverID = nowSelectedID
        con.workContent = nowContent
        con.resultDefine = nowResult
        con.promisePunish = nowPromise
        con.selfItem = nowSegementItem
